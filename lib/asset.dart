...
class _AssetMenuState extends State<AssetMenu> {
  ...
  void _addOrEditAsset({int? index}) {
    if (index != null) {
      ...
      _assetValueController.text =
          _assetNominal.toString().replaceAll(RegExp(r'\.0+$'), '');
      // Format the number with thousand separators
      if (_assetNominal > 0) {
        String formattedValue =
            NumberFormat('#,###').format(_assetNominal.toInt());
                      ...
                      onChanged: (value) {
                        ...
                        if (cleanedValue.isNotEmpty) {
                          ...
                          if (formattedValue != value) {
                            ...
                            int cursorPosition =
                                _assetValueController.selection.start;
                            // Calculate how many separators were before the cursor
                            int oldSeparatorsBeforeCursor = value
                  ...
                  if (_formKey.currentState!.validate()) {
                    ...
                    if (index == null) {
                      ...
                      await AssetDatabase.instance.logAssetChange(
                        ...
                        currency: _currency,
                        description:
                            'Created new $_assetType asset "$_assetName" with value ${formatCurrency(_assetNominal, _currency)}',
                      );
                    } else {
                      ...
                      if (oldAsset['name'] != _assetName) {
                        changes
                            .add('name: "${oldAsset['name']}" → "$_assetName"');
                      }
                      if (oldAsset['type'] != _assetType) {
                        changes
                            .add('type: "${oldAsset['type']}" → "$_assetType"');
                      }
                      if (oldAsset['nominal'] != _assetNominal) {
                        changes.add(
                            'value: ${formatCurrency(oldAsset['nominal'], oldAsset['currency'])} → ${formatCurrency(_assetNominal, _currency)}');
                      }
                      if (oldAsset['currency'] != _currency) {
                        changes.add(
                            'currency: "${oldAsset['currency']}" → "$_currency"');
                      }
  ...
  void _deleteAsset(int index) {
    showDialog(
      ...
      builder: (context) => AlertDialog(
        ...
        content:
            Text('Are you sure you want to delete ${_assets[index]['name']}?'),
        actions: [
          TextButton(
            onPressed: () => Navigator.pop(context),
          ...
          TextButton(
            ...
            onPressed: () async {
              ...
              await AssetDatabase.instance.logAssetChange(
                ...
                description:
                    'Deleted ${asset['type']} asset "${asset['name']}" with value ${formatCurrency(asset['nominal'], asset['currency'])}',
              );
  ...
  Widget _buildTotalAssetsWidget() {
    ...
    final Map<String, Map<String, double>> subTotals =
        _getSubTotalsByTypeAndCurrency();
          ...
          children: [
                      ...
                      builder: (context) => AlertDialog(
                        ...
                        content: const SingleChildScrollView(
                          child: ListBody(
                            children: [
                              Text(
                                  'This section shows the total value of all your assets grouped by currency.'),
                              SizedBox(height: 8),
                              Text(
                                  '• Each currency is displayed separately as they cannot be directly combined.'),
                              SizedBox(height: 8),
                              Text(
                                  '• Below each currency total, you can see a breakdown by asset type (Transactional, Savings, and Investment).'),
                              SizedBox(height: 8),
                              Text(
                                  '• The percentage shows how much of your portfolio each type represents within that currency.'),
                            ],
                          ),
                        ),
                        actions: [
            ...
            if (totals.isEmpty)
              ...
              ...totals.entries.map((entry) {
                ...
                final Map<String, double>? currencySubTotals =
                    subTotals[currency];
                        ...
                        child: Row(
                          children: [
                            ...
                            Container(
                              ...
                              decoration: BoxDecoration(
                                color:
                                    _typeColors[type]!.withValues(alpha: 0.1),
      ...
      builder: (context) => AlertDialog(
        ...
        content: Column(
          ...
          children: [
            ...
            Text(
                'Value: ${formatCurrency(asset['nominal'], asset['currency'])}'),
            Text('Currency: ${asset['currency']}'),
            Text(
                'Date: ${DateFormat(DateFormatManager.currentFormat).format(DateTime.parse(asset['date']))}'),
          ],
        ),
        actions: [
          TextButton(
          ...
          TextButton(
            ...
            onPressed: () async {
              ...
              if (selectedAssets.length == 1) {
                ...
                double totalValue = 0;
                String primaryCurrency = selectedAssets.isNotEmpty
                    ? selectedAssets.first['currency']
                    : 'USD';

                for (final asset in selectedAssets) {
                  typeCount[asset['type']] =
                      (typeCount[asset['type']] ?? 0) + 1;
                  if (asset['currency'] == primaryCurrency) {
                    totalValue += asset['nominal'];
                  }
                }

                String typesSummary = typeCount.entries
                    .map((e) => '${e.value} ${e.key}')
                    .join(', ');
                ...
                await AssetDatabase.instance.logAssetChange(
                  ...
                  description:
                      'Bulk deleted ${selectedAssets.length} assets ($typesSummary) with combined value ${formatCurrency(totalValue, primaryCurrency)}',
            ...
            : _assets.isEmpty
                ...
                : SingleChildScrollView(
                    padding: const EdgeInsets.only(
                        bottom:
                            120), // Add extra bottom padding for FAB clearance
                    child: Column(
                      crossAxisAlignment: CrossAxisAlignment.start,
                      children: [
                        ...
                        ListView.builder(
                          padding: const EdgeInsets.all(8),
                          shrinkWrap:
                              true, // Important to make ListView work inside ScrollView
                          physics:
                            ...
                            return Card(
                              ...
                              shape: RoundedRectangleBorder(
                                ...
                                side: BorderSide(
                                  ...
                                  width: _selectedAssetIds.contains(asset['id'])
                                      ? 2
                                      : 1,
                                ),
                              ),
                              child: InkWell(
                                onTap: _isSelectionMode
                                    ...
                                    : () => _showAssetDetails(
                                        asset), // Make card tappable to show details
                                          ...
                                          Tooltip(
                                            ...
                                            child: Container(
                                              constraints: const BoxConstraints(
                                                  maxWidth:
                                                      120), // More constrained
                                              child: Text(
                                                formatCurrency(asset['nominal'],
                                                    asset['currency']),
                                                style: const TextStyle(
